...
const Questionnaire = () => {
  ...
  const { user } = state || {}; // ✅ Handle potential `null` user

  useEffect(() => {
    if (!user || !user._id) return; // ✅ Prevent accessing `_id` if `user` is null
    ...
    fetchResponses();
  }, [user?._id]); // ✅ Safe access with optional chaining
  ...
  const handleSubmit = async () => {
    if (!user || !user._id) return toast.error("❌ User not authenticated."); // ✅ Prevent error if user is missing
